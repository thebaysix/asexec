...
The full context/blind-spots/brainstorm/interview/plan/notes live in `crystallize/` (00–07, gitignored — local only) and, name-stripped, in **GitHub issue #1** (`lwhitestone/asexec`). This repo is an active *crystallize* effort (see `.crystallize.json`): when implementing against `crystallize/04-plan.md`, log deviations to `crystallize/05-notes.md` under "Decisions & deviations". Named v2 items (out of scope for now): OpenTimestamps/Bitcoin cryptographic ceiling, re-execution/determinism mode, hash-log anchoring, web-of-trust / key transparency.
